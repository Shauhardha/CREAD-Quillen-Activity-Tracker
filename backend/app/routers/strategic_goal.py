...
# Create
@router.post("/", response_model=StrategicGoalOut, dependencies=[Depends(require_admin)])
...
def get_goals(db: Session = Depends(get_db)):
    return db.query(StrategicGoal).filter(StrategicGoal.deleted_at == None).all()

# Update
@router.put("/{goal_id}", response_model=StrategicGoalOut, dependencies=[Depends(require_admin)])
...
# Soft-delete
@router.delete("/{goal_id}", dependencies=[Depends(require_admin)])
